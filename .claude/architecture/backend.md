...
| MFA enrolment | Guard, `Organization.requireMfa` | **Registered (Task 12)**, `MfaEnrolmentGuard` — written in Task 11 and left in no module, placed here ahead of Authorize so a member with no factor hears `MFA_ENROLMENT_REQUIRED` rather than `PERMISSION_DENIED`. **Can refuse as of Task 13**, which shipped both the routes that declare a permission and the endpoint that creates an organisation. `MFA_ENROLMENT_REQUIRED` has a reachable producer for the first time, proved in both directions. It still exits early when the request names no organisation, which is what keeps a member with no factor able to reach their own MFA enrolment, session document and logout |
