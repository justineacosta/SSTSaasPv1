...
- Prefixes are a closed set in `ID_PREFIXES`: `org`, `usr`, `mbr`, `ses`, `crd`, `rol`, `prm`,
  ...
  use them, so the two prefixes most likely to be needed first do not arrive as a breaking
  rename. **This is not the whole vocabulary and is not meant to be** — the Phase 0 API documents
  already write `prj_`, `ast_` and `evt_` (`api/filtering.md` §1, `api/webhooks.md` §2), none of
  which are registered yet. Each is added by the phase that creates its model.
- Every **single-column** primary key in `packages/db/prisma/schema.prisma` is declared
  `String @id` with **no `@default`**, and lands in Postgres as `"id" TEXT NOT NULL`. That is 9
  of the 10 models; the tenth, `RolePermission`, is a pure join table with a composite key
  (`@@id([roleId, permissionId])`) and no surrogate ID of its own. There is no database-side
...
- **The illustrative IDs in the Phase 0 documents are not real IDs, and are deliberately left
  ...
  `newId`, not typed.
- **Hand-typed example IDs drift from the encoding, and one already had.** At the time of
  writing, `id.ts`'s own docstring carried a 25-character example containing `U`, `I` and `O` —
  all excluded by the Crockford alphabet the same file defines — so `parseIdPrefix` returned
  `undefined` for it. That is a defect in the docstring, not in this decision, and **fixing it
  does not require revisiting this ADR**: the rule that survives is that any example intended to
  be exact must come from `newId`, never from a keyboard.
