...
describe('TenantContext', () => {
  ...
  it('exposes permissions as a genuinely read-only set', () => {
    ...
    // has no `add`, so this line is a compile error — and `@ts-expect-error`
    // turns "compile error" into a test that fails loudly if the type is ever
    // relaxed to `Set<Permission>`.
    // @ts-expect-error ReadonlySet has no `add`; widening a live permission set is the bug this prevents.
    expect(() => context.permissions.add('organization.delete')).toBeDefined();
  });
